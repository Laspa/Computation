...
#  June 1, 2016
#  Script to run elastic constants calculations on Stampede
#===============================================================================
""" 
...
EMAIL = 'jlkaufman@hmc.edu' # email address for SLURM
...
def prepareDirectories(system, a0, Cqq, tList, rList, isif=2):
	...
	if not os.path.exists(Cqq): os.makedirs(Cqq)
	...
	for t in tList:
		...
		if len(rList) > 1:
			for r in rList:
				rDir = '%s/%s'%(path,r) 
				if not continuation: startDir = rDir
